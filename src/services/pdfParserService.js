class PDFParserService {
  constructor() {
    ...
    this.maxTextLength = 50000; // Increase limit for better content extraction
    this.pdfjsLib = null;
  }
  ...
  async initialize() {
    ...
    try {
      // Try to load the modern build first
      const pdfjs = await import('pdfjs-dist/build/pdf.min.mjs');
      this.pdfjsLib = pdfjs;
      console.log('PDF.js library loaded successfully');
      return true;
    } catch (error) {
      console.warn('Failed to load modern PDF.js library:', error.message);
      try {
        // Fallback to legacy build for Node.js environments
        const pdfjs = await import('pdfjs-dist/legacy/build/pdf.min.mjs');
        this.pdfjsLib = pdfjs;
        console.log('PDF.js legacy library loaded successfully');
        return true;
      } catch (legacyError) {
        console.warn('Failed to load legacy PDF.js library:', legacyError.message);
        return false;
      }
    }
  }
